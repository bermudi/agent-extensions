...
export function resolveModelOrThrow(
  ...
  if (idx === -1) {
    ...
    throw new Error(
      `Unknown model id "${spec}". No match in available models.`,
    );
  }
...
async function callModel(
  ...
  try {
    ...
    if (!auth.ok)
      throw new Error((auth as { error?: string }).error ?? "no API key");
...
export class ArenaVotingUI {
  ...
  handleInput(data: string): void {
    if (matchesKey(data, Key.up) || matchesKey(data, "k")) {
      ...
      if (this.selected < this.responses.length - 1) {
        this.selected++;
        this.invalidate();
      }
  ...
  render(width: number): string[] {
    ...
    lines.push(
      t.fg("accent", t.bold("🏟  Arena — Vote for the best response")),
    );
    ...
    for (let i = 0; i < this.responses.length; i++) {
      ...
      const stats = t.fg(
        "dim",
        `${fmtDuration(r.durationMs)} · ${fmtTokens(r.tokens)} tokens`,
      );
      ...
      if (r.error) {
        ...
        if (textLines.length > maxLines) {
          lines.push(
            t.fg("dim", `  … ${textLines.length - maxLines} more lines`),
          );
        }
      }
      lines.push("");
...
export default function arenaExtension(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("arena", {
    ...
    handler: async (args, ctx) => {
      ...
      if (available.length < 3) {
        ctx.ui.notify(
          "Need at least 3 configured models to run arena",
          "error",
        );
        return;
      }
      ...
      const modelA = await ctx.ui.select(
        "Select Model A:",
        modelOptions.map((m) => m.value),
      );
      if (!modelA) return;

      const remainingB = modelOptions.filter((m) => m.value !== modelA);
      const modelB = await ctx.ui.select(
        "Select Model B:",
        remainingB.map((m) => m.value),
      );
      if (!modelB) return;

      const remainingC = remainingB.filter((m) => m.value !== modelB);
      const modelC = await ctx.ui.select(
        "Select Model C:",
        remainingC.map((m) => m.value),
      );
      if (!modelC) return;
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      prompt: Type.String({
        description: "The prompt to send to all 3 models.",
      }),
      modelA: Type.String({
        description:
          "First model, e.g. 'anthropic/claude-sonnet-4' or bare id like 'gpt-4.1'",
      }),
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      ...
      const fireProgress = () =>
        onUpdate?.({
          ...
          details: {
            ...
            progress,
            revealed: false,
          } satisfies ArenaProgressDetails,
        });
      fireProgress();
      ...
      return {
        content: [
          { type: "text", text: formatResults(responses, vote, winner) },
        ],
        details: {
          prompt,
          responses,
    ...
    renderCall(args, theme, _ctx) {
      ...
      const lines = [theme.fg("toolTitle", theme.bold("arena"))];
      for (let i = 0; i < 3; i++) {
    ...
    renderResult(result, options, theme, _ctx) {
      ...
      if (details.vote) {
        lines.push(
          theme.fg("success", `🗳  ${details.vote} wins!`) +
            " " +
            theme.fg("muted", `(${details.winner})`),
        );
      } else {
      ...
      for (let i = 0; i < details.responses.length; i++) {
        ...
        const icon = r.error
          ? theme.fg("error", "✗")
          : theme.fg("success", "✓");
        const reveal = details.revealed
          ? ` ${theme.fg("dim", "→")} ${theme.fg("accent", r.identity)}`
          : "";
        const stats = theme.fg(
          "dim",
          `${fmtDuration(r.durationMs)} · ${fmtTokens(r.tokens)} tok`,
        );

        lines.push(
          `${treeChar(i, details.responses.length)} ${icon} ${theme.bold(r.label)}${reveal}  ${stats}`,
        );
        ...
        if (r.error) {
          ...
          for (const line of outputLines.slice(0, maxLines)) {
            lines.push(
              `${ind}${theme.fg("toolOutput", truncateToWidth(line, TRUNCATE_WIDTH))}`,
            );
          }
          const remaining = outputLines.length - maxLines;
          if (remaining > 0) {
            lines.push(
              `${ind}${theme.fg("muted", `… ${remaining} more lines`)}`,
            );
          }
        }
      }
