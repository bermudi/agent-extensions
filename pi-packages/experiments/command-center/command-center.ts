  ...
  for (const dir of dirs) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        // Parse header
        let header: { id: string; timestamp: string; cwd: string } | null =
          null;
        try {
...
class CommandCenterComponent {
  ...
  private handleSessionsInput(data: string): void {
    ...
    if (matchesKey(data, "up")) {
      ...
      this.sessionSelected = Math.min(
        sessions.length - 1,
        this.sessionSelected + 1,
      );
      this.tui.requestRender();
    } else if (matchesKey(data, "return") || matchesKey(data, "enter")) {
      const s = sessions[this.sessionSelected];
  ...
  private renderLoading(): string[] {
    const { row, emptyRow, topBorder, bottomBorder, divider } = makeBox(
      BOX_WIDTH - 2,
      this.theme,
    );
    ...
    lines.push(
      row("  " + muted("Reading usage data from ~/.pi/agent/sessions/")),
    );
    lines.push(emptyRow());
    lines.push(emptyRow());
    lines.push(divider());
  ...
  private renderDashboard(): string[] {
    ...
    // Big stats
    lines.push(
      row(`  ${bold("Sessions:")}  ${accent(String(d.totalSessions))}`),
    );
    lines.push(
      row(
        `  ${bold("Tokens:")}    ${accent(fmtNum(d.totalUsage.totalTokens))}  (in ${fmtNum(d.totalUsage.input)} / out ${fmtNum(d.totalUsage.output)})`,
      ),
    );
    lines.push(
      row(`  ${bold("Cost:")}       ${success(fmtCost(d.totalUsage.cost))}`),
    );
    if (d.totalUsage.cacheRead > 0 || d.totalUsage.cacheWrite > 0) {
      lines.push(
        row(
          `  ${bold("Cache:")}     read ${fmtNum(d.totalUsage.cacheRead)} / write ${fmtNum(d.totalUsage.cacheWrite)}`,
        ),
      );
    }
    lines.push(emptyRow());
    ...
    for (const p of top) {
      ...
      lines.push(
        row(
          `  ${accent(projPadded)}  ${bold(tokStr)}  ${success(costStr)}  ${dim(sessStr)}`,
        ),
      );
    }
    ...
    lines.push(
      row(
        `  ${accent("enter")} ${dim("projects")}  ${accent("tab")} ${dim("switch tab")}  ${accent("esc")} ${dim("close")}`,
      ),
    );
    lines.push(bottomBorder());
    return lines;
  ...
  private renderProjects(): string[] {
    ...
    // Header
    const sortLabel =
      this.projectSortKey === "tokens"
        ? "tokens"
        : this.projectSortKey === "cost"
          ? "cost"
          : "sessions";
    const sortArrow = this.projectSortDir === "desc" ? "↓" : "↑";
    lines.push(
      row(
        `  ${bold("Project".padEnd(22))} ${bold("Sessions".padStart(10))}  ${bold("Tokens".padStart(10))} ${sortArrow}  ${bold("Cost".padStart(8))}`,
      ),
    );
    ...
    if (projects.length === 0) {
      ...
      for (let i = startIdx; i < endIdx; i++) {
        ...
        const tokens = isSel
          ? accent(fmtNum(p.usage.totalTokens).padStart(10))
          : muted(fmtNum(p.usage.totalTokens).padStart(10));
        const cost = isSel
          ? success(fmtCost(p.usage.cost).padStart(8))
          : muted(fmtCost(p.usage.cost).padStart(8));
        const sessions = String(p.sessionCount).padStart(10);

        lines.push(
          row(` ${prefix} ${proj.padEnd(22)} ${sessions}  ${tokens}  ${cost}`),
        );
      }
      ...
      lines.push(
        row(
          dim(
            `  ${sel + 1}/${projects.length} projects  sorted by: ${sortLabel} ${sortArrow}`,
          ),
        ),
      );
    }
    ...
    lines.push(
      row(
        `  ${accent("t/c/s")} ${dim("sort")}  ${accent("↑↓")} ${dim("nav")}  ${accent("enter")} ${dim("sessions")}  ${accent("tab")} ${dim("switch tab")}  ${accent("esc")} ${dim("close")}`,
      ),
    );
    lines.push(bottomBorder());
    return lines;
  ...
  private renderSessions(): string[] {
    ...
    if (this.deleting) {
      ...
      lines.push(
        row(
          `  ${accent("y")} ${dim("yes")}  ${accent("any other")} ${dim("cancel")}`,
        ),
      );
      lines.push(emptyRow());
      ...
      for (let i = startIdx; i < endIdx; i++) {
        ...
        lines.push(
          row(
            ` ${prefix} ${accent(proj.padEnd(18))} ${dim(date.padEnd(16))} ${bold(tokens.padStart(6))}`,
          ),
        );
        if (title) {
          lines.push(
            row(`   ${muted(truncateToWidth(title, BOX_WIDTH - 8, "…"))}`),
          );
        }
      }
    ...
    lines.push(
      row(
        `  ${accent("enter")} ${dim("read")}  ${accent("r")} ${dim("resume")}  ${accent("d")} ${dim("delete")}  ${accent("tab")} ${dim("switch tab")}  ${accent("esc")} ${dim("close")}`,
      ),
    );
    lines.push(bottomBorder());
    return lines;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Get aggregate session statistics and per-project token/cost breakdown",
    promptGuidelines: [
    ...
    async execute() {
      ...
      const text = [
        ...
        data.totalUsage.cacheRead > 0
          ? `- **Cache read:** ${fmtNum(data.totalUsage.cacheRead)}`
          : null,
        data.totalUsage.cacheWrite > 0
          ? `- **Cache write:** ${fmtNum(data.totalUsage.cacheWrite)}`
          : null,
        "",
        "**Top projects by tokens:**",
        ...projectLines,
  ...
  pi.registerCommand("cc", {
    description:
      "Open the Command Center — session dashboard with tokens, projects, and session browser",
    handler: async (_args, ctx) => {
      ...
      if (action.type === "read") {
        ...
        ctx.ui.setEditorText(
          `Read this session file with session_read:\nfile: ${action.session.file}`,
        );
        ctx.ui.notify(
          `Session ${action.session.id.slice(0, 8)} — reading`,
          "info",
        );
        return;
      }
      ...
      if (action.type === "delete") {
        const ok = await ctx.ui.confirm(
          "Delete Session",
          `Delete "${action.session.name || action.session.firstUserMessage.slice(0, 50)}"?`,
        );
        if (ok) {
          const success = await deleteSession(action.session.file);
          ctx.ui.notify(
            success ? "Session deleted" : "Failed to delete session",
            success ? "info" : "error",
          );
        }
        return;
