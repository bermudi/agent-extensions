      ...
      return {
        render(width: number): string[] {
          ...
          for (const entry of entries) {
            ...
            lines.push(
              bg(pad(`${icon} ${theme.bold(entry.key)}  ${status}`, width)),
            );
      ...
      return {
        render(width: number): string[] {
          ...
          lines.push(
            bg(
              pad(
                theme.fg(
                  "muted",
                  ` Select 2+ models (${checked.size} selected) `,
                ),
                width,
              ),
            ),
          ...
          for (let i = start; i < end; i++) {
            ...
            lines.push(
              bg(
                pad(`${cursor}${theme.fg(color, `${prefix} ${label}`)}`, width),
              ),
            );
          }
          ...
          if (available.length > maxVisible) {
            lines.push(
              bg(
                pad(
                  theme.fg("dim", `  … ${available.length - maxVisible} more`),
                  width,
                ),
              ),
            );
...
async function runSource(
  ...
  const agent = new Agent({
    ...
    streamFn: async (m, context, options) => {
      ...
      if (!auth.ok)
        throw new Error(`Auth failed: ${(auth as { error: string }).error}`);
...
async function runFusion(
  ...
  const sourceSection = okSources
    .map(
      (s, i) =>
        `### Response ${i + 1} (${s.model.provider}/${s.model.id})\n\n${s.output}`,
    )
    .join("\n\n");

  const failedSection = failedSources.length
    ? failedSources
        .map((s) => `- ${s.model.provider}/${s.model.id}: ${s.error}`)
        .join("\n")
    : "";
  ...
  try {
    ...
    const analysisRe = new RegExp(
      `${delim.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}ANALYSIS\\s*([\\s\\S]*?)(?=${delim.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}FUSED|$)`,
    );
    const fusedRe = new RegExp(
      `${delim.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}FUSED\\s*([\\s\\S]*)`,
    );
...
export default function fusionExtension(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("fusion", {
    ...
    handler: async (args, ctx) => {
      ...
      const currentSpec = ctx.model
        ? `${ctx.model.provider}/${ctx.model.id}`
        : fuseOptions[0]!;
      const orderedOptions = [
        currentSpec,
        ...fuseOptions.filter((s) => s !== currentSpec),
      ];
      ...
      const fuseModel =
        resolveModel(fuseSpec, ctx.modelRegistry, ctx.model) ??
        selection.models[0]!;
      ...
      const systemPrompt =
        ctx.getSystemPrompt() ?? "You are a helpful assistant.";
      ...
      if (ctx.hasUI) {
        const overlay = showFusionProgress(
          selection.models,
          abortController,
          ctx,
        );
        progress = {
          state: overlay.state,
          requestRender: overlay.requestRender,
          close: overlay.close,
        };
      ...
      try {
        ...
        const totalSourceDuration = sourceResults.reduce(
          (s, r) => s + r.durationMs,
          0,
        );
        const totalSourceTokens = sourceResults.reduce(
          (s, r) => s + r.tokens,
          0,
        );
        ...
        parts.push(
          `**Time:** ${fmtDuration(totalDuration)} · **Tokens:** ${fmtTokens(totalTokens)}`,
        );
        parts.push("");
        ...
        for (const src of sourceResults) {
          ...
          const status = ok
            ...
            : `failed · ${src.error!}`;
          parts.push(
            `### ${ok ? "✓" : "✗"} ${src.model.provider}/${src.model.id} — ${status}`,
          );
          if (ok && src.output) {
            parts.push("");
  ...
  pi.registerCommand("fusion-continue", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (!lastDetails) {
        if (ctx.hasUI)
          ctx.ui.notify("No fusion result found in this session", "warning");
        return;
      }
      ...
      if (ctx.hasUI) {
        ctx.ui.notify(
          "Fusion result injected into conversation context",
          "info",
        );
      }
    },
