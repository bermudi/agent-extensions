...
| `clean-tui`        | tool overrides (no command)   | Collapse built-in tool output for a cleaner TUI: same-tool calls within one assistant message share one block (e.g. `read ×2`), assistant messages always break the block, images stay visible without expanding, expand a row with ctrl+o to see results/diffs. Long bash commands get an AI-generated summary once you pick a model with `/goodies summary-model <provider/model>` (see "Smart summaries" below). While enabled, also flips `@bermudi/pi-codex`'s `apply_patch`/`web_search` into the same burst style. |
...
```bash
pi install npm:bermudis-pi-goodies@0.12.0
```
