...
export default function (pi: ExtensionAPI) {
	...
	let pollingState: "running" | "stopped" | "degraded" | "unauthorized" = "stopped";
	...
	function updateStatus(ctx: ExtensionContext, error?: string): void {
		...
		if (pollingState === "unauthorized") {
			ctx.ui.setStatus("telegram", `${label} ${theme.fg("error", "unauthorized")} ${theme.fg("muted", error ?? "token revoked")}`);
			return;
		}
		if (error) {
		...
		if (pollingState === "degraded") {
			ctx.ui.setStatus("telegram", `${label} ${theme.fg("warning", "degraded")} ${theme.fg("muted", "retrying…")}`);
			return;
		}
	...
	async function callTelegram<TResponse>(
		...
		return withRateLimit(() => withRetry(async () => {
			const response = await telegramFetch(`https://api.telegram.org/bot${config.botToken}/${method}`, {
				method: "POST",
	...
	async function flushPreview(chatId: number): Promise<void> {
		...
		try {
			...
			if (msg.includes("message to edit not found") || msg.includes("message not found") || msg.includes("message_id_invalid")) {
	...
	async function sendTextReply(chatId: number, replyToMessageId: number, text: string): Promise<number | undefined> {
		...
		for (const chunk of chunks) {
			...
			try {
				...
				const err = error instanceof Error ? error : new Error(String(error));
				const msg = err.message.toLowerCase();
				if (msg.includes("message to reply not found") || msg.includes("message_id_invalid") || msg.includes("reply message not found") || msg.includes("bad request: message to reply not found")) {
	...
	async function sendQueuedAttachments(turn: ActiveTelegramTurn): Promise<void> {
		for (const attachment of turn.queuedAttachments) {
			try {
				...
				try { await unlink(attachment.path); } catch { /* ignore */ }
			}
		}
	}
	...
	async function promptForConfig(ctx: ExtensionContext): Promise<void> {
		...
		try {
			...
			const response = await telegramFetch(`https://api.telegram.org/bot${nextConfig.botToken}/getMe`);
	...
	async function handleUpdate(update: TelegramUpdate, ctx: ExtensionContext): Promise<void> {
		...
		if (processedUpdateIds.size > MAX_UPDATE_ID_HISTORY) {
			const toDelete = Array.from(processedUpdateIds).slice(0, processedUpdateIds.size - MAX_UPDATE_ID_HISTORY);
	...
	async function pollLoop(ctx: ExtensionContext, signal: AbortSignal): Promise<void> {
		...
		try {
			await callTelegram("deleteWebhook", { drop_pending_updates: false }, { signal, timeout: API_CALL_TIMEOUT_MS });
		} catch {
			// ignore
		}
		...
		while (!signal.aborted) {
			try {
				...
				if (code === 401 || code === 403 || message.includes("401") || message.includes("403") || message.includes("Unauthorized")) {
				...
				const base = Math.min(1000 * Math.pow(2, consecutiveFailures - 1), MAX_BACKOFF_MS);
				const jitter = Math.random() * 500;
