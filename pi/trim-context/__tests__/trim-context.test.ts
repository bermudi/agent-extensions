...
function makeBashExecution(command: string, output: string, timestamp = 4000): AnyMessage {
	return { role: "bashExecution", command, output, exitCode: 0, cancelled: false, truncated: false, timestamp };
}
...
describe("extractText", () => {
	...
	it("ignores non-text blocks", () => {
		expect(
			extractText([{ type: "image", data: "..." }, { type: "text", text: "hello" }]),
		).toBe("hello");
	});
