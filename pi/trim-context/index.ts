...
function hashText(text: string): string {
	return createHash("sha256").update(text.normalize().slice(0, 500)).digest("hex").slice(0, 16);
}
		...
		for (const msg of turn.messages) {
			...
			if (role === "user") {
				...
				const toolCalls = (msg.content || [])
					...
					.map((c: AnyMessage) => `${c.name}(${JSON.stringify(c.arguments).slice(0, 200)})`)
					.join("; ");
				...
				parts.push(`[Previous Summary]: ${String(msg.summary || "").slice(0, 500)}`);
			}
		}
		parts.push("");
	}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			action: Type.Union([Type.Literal("list"), Type.Literal("compact")], {
				description: "'list' to see available turns, 'compact' to compact specific turns by index",
			}),
			indices: Type.Optional(
				Type.Array(Type.Number(), {
					description: "0-based turn indices to compact (from 'list' output). Required for action='compact'.",
				}),
			),
			focus: Type.Optional(
				Type.String({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (action === "list") {
				...
				return {
					content: [{ type: "text", text: `Conversation turns:\n${lines.join("\n")}` }],
				};
			}
			...
			if (indices.length === 0) {
				return {
					content: [{ type: "text", text: "No indices provided. Use action='list' first to see available turns, then pass indices to compact." }],
					isError: true,
				};
			}

			// Validate indices
			const invalid = indices.filter((i: number) => i < 0 || i >= allTurns.length);
			if (invalid.length > 0) {
				return {
					content: [{ type: "text", text: `Invalid turn indices: ${invalid.join(", ")}. Valid range: 0-${allTurns.length - 1}` }],
					isError: true,
				};
			}
			...
			if (indices.includes(lastIdx)) {
				return {
					content: [{ type: "text", text: `Cannot compact the current turn (index ${lastIdx}). Remove it from indices.` }],
					isError: true,
				};
			}

			// Deduplicate and sort
			const uniqueIndices = [...new Set(indices)].sort((a: number, b: number) => a - b);
			...
			if (toCompact.length === 0) {
				return {
					content: [{ type: "text", text: `All selected turns are already compacted.${alreadyDone.length ? ` Indices: ${alreadyDone.join(", ")}` : ""}` }],
				};
			}

			onUpdate?.({
				content: [{ type: "text", text: `Generating summary for ${toCompact.length} turn(s) at indices [${toCompact.map((t) => t.index).join(", ")}]...` }],
			});

			try {
				const summary = await generateSummary(toCompact, focus, signal, ctx as AnyMessage);

				if (!summary) {
					return {
						content: [{ type: "text", text: "Summary was empty. No changes made." }],
						isError: true,
					};
				}
				...
				const totalMsgs = toCompact.reduce((s: number, t: Turn) => s + t.messages.length, 0);
				const tokensStr = usage?.tokens
					? ` (~${usage.tokens.toLocaleString()} tokens in context)`
					: "";

				const skipped = alreadyDone.length > 0
	...
	pi.registerCommand("trim", {
		...
		handler: async (args, ctx) => {
			...
			try {
				const summary = await generateSummary([turn], undefined, undefined, ctx as AnyMessage);
				...
				ctx.ui.notify(`Turn ${idx + 1} compacted. Use /untrim to restore.`, "info");
			} catch (err) {
				const msg = err instanceof Error ? err.message : String(err);
