...
function buildTreeOverview(entries: SessionEntry[]): string {
  ...
  function renderNode(
    ...
    if (entry.type === "message" && entry.message?.role === "user") {
      ...
      lines.push(
        `${prefix}${connector}Turn ${userTurnIdx}${branchMarker}: ${preview}`,
      );
    } else if (entry.type === "compaction") {
      lines.push(
        `${prefix}${connector}[compaction: ${entry.tokensBefore} tokens]`,
      );
...
function chunkIntoPhases(transcript: string): Phase[] {
  ...
  if (
    boundaries.length === 0 ||
    (boundaries.length === 1 && boundaries[0].idx === 0)
  ) {
...
function extractStats(entries: SessionEntry[]): string {
  ...
  if (totalTokens > 0)
    stats.push(`Total tokens: ${(totalTokens / 1000).toFixed(0)}k`);
...
async function synthesizeSummary(
  ...
  const prompt = SYNTHESIS_PROMPT.replace("{tree}", treeOverview).replace(
    "{summaries}",
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("summarize", {
    description:
      "Summarize a (possibly branched) session — current or by path/UUID",
    ...
    handler: async (args, ctx) => {
      ...
      if (!sessionPath) {
        ctx.ui.notify(
          "No session found. Provide a path or UUID, or run inside a session.",
          "error",
        );
        return;
      }
      ...
      ctx.ui.notify(
        `Session: ${entries.length} entries, building transcript...`,
        "info",
      );
