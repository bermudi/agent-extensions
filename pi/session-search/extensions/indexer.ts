...
function findSessionFiles(
  sessionsDir: string,
): { path: string; dirName: string; filename: string }[] {
...
async function extractContentAsync(
  filePath: string,
): Promise<{ chunks: string[]; firstUserMessage: string | null; cwd: string }> {
  const chunks: string[] = [];
...
export async function updateIndex(
  ...
  const rows = db.prepare("SELECT path, mtime_ms FROM sessions").all() as {
    path: string;
    mtime_ms: number;
  }[];
  ...
  if (removedPaths.length > 0) {
    ...
    const deleteFts = db.prepare(
      "DELETE FROM session_fts WHERE session_path = ?",
    );
  ...
  onProgress?.(
    `Indexing ${toIndex.length} session${toIndex.length > 1 ? "s" : ""}...`,
  );
  ...
  const insertFts = db.prepare(
    "INSERT INTO session_fts (content, session_path) VALUES (?, ?)",
  );

  // Process in batches: async file reads → sync DB writes → yield
  for (
    let batchStart = 0;
    batchStart < toIndex.length;
  ...
  onProgress?.(
    `Indexed ${toIndex.length} session${toIndex.length > 1 ? "s" : ""}`,
  );
