...
export function renderPromptInput(
  ...
  lines.push(
    row(
      `  ${bold(accent("📂"))} ${accent(projectStr)}  ${dim(`→ ${actionLabel}`)}`,
    ),
  );
  ...
  if (!state.customPrompt) {
    lines.push(
      row(
        `${prefixStr}${cursor}${muted("e.g. focus on the auth implementation decisions...")}`,
      ),
    );
  } else {
    ...
    for (let i = 0; i < maxLines && i * textW < text.length + 1; i++) {
      ...
      if (
        state.cursorPos >= sliceStart &&
        state.cursorPos <= Math.min(sliceEnd, text.length) &&
        sliceStart <= text.length
      ) {
