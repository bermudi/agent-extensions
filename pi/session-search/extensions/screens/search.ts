...
export function handleSearchInput(
  ...
  if (
    ...
    return {
      type: "queryChanged",
      query: newQuery,
      cursorPos: state.cursorPos + data.length,
    };
  }
...
export function renderSearch(
  ...
  const { row, emptyRow, divider, topBorder, bottomBorder } = makeBox(
    innerW,
    theme,
  );
  ...
  if (!state.query.trim() && state.results.length === 0) {
    ...
    const startIdx = Math.max(
      0,
      Math.min(
        state.selected - Math.floor(maxVisible / 2),
        state.results.length - maxVisible,
      ),
    );
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      lines.push(
        row(
          `  ${prefix} ${isSel ? bold(accent(projectStr)) : projectStr}  ${dim(dateStr)}`,
        ),
      );
      ...
      if (r.title) {
        ...
        const titleClean = r.title
          ...
          .trim();
        lines.push(
          row(`    ${muted(truncateToWidth(titleClean, titleMaxW, "…"))}`),
        );
      }
    ...
    if (state.results.length > maxVisible) {
      lines.push(
        row(dim(`  ${state.selected + 1}/${state.results.length} ${label}`)),
      );
    }
  }
